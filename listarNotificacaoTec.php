...
<html>
    ...
    <body>
        ...
        if (isset($_GET['id'])) {
            ...
            if (empty($_SESSION['idDados' . $id][0])) {
                ...
                $_SESSION['cpf_perfilCliente'] = $_SESSION['cpfTec'];
            } else {
                $cont = $id;
                $_SESSION['idContrato'] = $_SESSION['idDados' . $id][0];
                $_SESSION['cpfCli'] = $_SESSION['idDados' . $id][1];
                $_SESSION['cpf_perfilCliente'] = $_SESSION['cpfCli'];
            }
        }
        ...
        $div = 'divid';
        $notificacoes = listarNotificacaoTec($conexao, $cpf, 'cpf_tecnico', $_SESSION['idContrato'], 'id');
        ...
        <center>
            ...
            <table border = "1">
                <tr>
                    ...
                    <td>Cancelar</td>
                    <?php
                    if (($notificacao['estadoContrato'] != 'ConfirmadoTecnico')&& ($notificacao['estadoContrato'] != 'Em analise')) {
                ...
                <tr>
                    ...
                    $_SESSION['cpfCli' . $cont] = $notificacao['cpf_tecnico'];
                    $_SESSION['idContrato' . $cont] = $notificacao['id_contrato'];
                    ?>
                    <?php $_SESSION['cpf_perfilCliente'] = $notificacao['cpf_cliente']; ?>
                </tr>
            </table>
        </center>
        <?php
    ...
    if (isset($_GET['estado'])) {
        ...
        if (!empty($estadoContrato)) {
            if ($estadoContrato === 'conf') {
                if (atualizarEstadoContratoTec($conexao, $_SESSION['cpfCli'], $cpf, $estadoContrato)) {
                    echo '<script> var r = confirm("Confirmado!")
                            if (r === true) {
                ...
                if (atualizarEstadoContratoTec($conexao, $_SESSION['cpfCli'], $cpf, $estadoContrato, $_SESSION['idContrato'])) {
                ...
                if (atualizarEstadoNot($conexao, $_SESSION['idContrato'], 'tec')) {
    ...
    <script>
        function atualizar() {
            var estadoCont = "conf";
            window.location = "listarNotificacaoTec.php?id=" +<?php echo $_SESSION['id']; ?> + "&estado=" + estadoCont;
        ...
        function visualizarNt(cont) {
            window.location = "listarNotificacaoTec.php?id=" +<?php echo $_SESSION['id']; ?> + "&estado=" + cont;
